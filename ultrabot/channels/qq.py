"""QQ Bot channel implementation using the botpy SDK.

Uses WebSocket (via botpy) -- supports C2C (private) and group @-mention messages.

Inbound:
    - Parse QQ botpy messages (C2C / Group)
    - Download attachments to media dir using chunked streaming
    - Publish to ultrabot bus
Outbound:
    - Send attachments via QQ rich media API (base64 upload + msg_type=7)
    - Send text (plain or markdown)

Requirements:
    - ``qq-botpy`` package (``pip install 'ultrabot-ai[qq]'``)
    - App ID and Secret from the `QQ Open Platform`_

.. _QQ Open Platform: https://q.qq.com/
"""
...
def _make_bot_class(channel: "QQChannel") -> "type[botpy.Client]":
    ...
    class _Bot(botpy.Client):  # type: ignore[misc]
...
class QQChannel(BaseChannel):
    """QQ Bot channel using botpy SDK with WebSocket connection.
    ...
    appId               App ID from QQ Open Platform.
    secret              App secret from QQ Open Platform.
    allowFrom           List of user IDs allowed to interact.
    msgFormat            ``"plain"`` (default) or ``"markdown"``.
    downloadChunkSize   Chunk size for attachment downloads (default 256 KB).
    downloadMaxBytes    Max download size safety limit (default 200 MB).
    ==================  ====================================================
    """
    ...
    def __init__(self, config: dict, bus: "MessageBus") -> None:
        ...
        self._msg_format: Literal["plain", "markdown"] = config.get(
            "msgFormat", config.get("msg_format", "plain")
        )
        self._download_chunk_size: int = config.get(
            "downloadChunkSize", config.get("download_chunk_size", 256 * 1024)
        )
        self._download_max_bytes: int = config.get(
            "downloadMaxBytes", config.get("download_max_bytes", 200 * 1024 * 1024)
        )
    ...
    async def start(self) -> None:
        ...
        logger.info("QQ channel started (C2C & Group supported)")
        await self._run_bot()
    ...
    async def _on_message(self, data: Any, is_group: bool = False) -> None:
        """Parse inbound message, download attachments, publish to bus."""
        ...
        if not self._is_allowed(user_id):
            logger.warning("QQ message from disallowed user {}", user_id)
            return
        ...
        if recv_lines:
            ...
            content = f"{content}\n\n{file_block}".strip() if content else file_block
        ...
        logger.debug("QQ inbound from {}: {}", user_id, content[:80])
        await self.bus.publish(inbound)

    async def _handle_attachments(
        self, attachments: list[Any]
    ) -> tuple[list[str], list[str], list[dict[str, Any]]]:
        """Extract, download, and format attachments."""
        ...
        for att in attachments:
            ...
            local_path = await self._download_to_media_dir(url, filename_hint=filename)
            att_meta.append(
                {
                    "url": url,
                    "filename": filename,
    ...
    async def _download_to_media_dir(
        ...
        """Download an inbound attachment using chunked streaming."""
        import aiohttp
        ...
        try:
            async with self._http.get(url, timeout=aiohttp.ClientTimeout(total=120), allow_redirects=True) as resp:
                if resp.status != 200:
                ...
                if target.exists():
                    target = self._media_dir / f"{target.stem}_{ts}{target.suffix}"

                tmp_path = target.with_suffix(target.suffix + ".part")

                # Stream write
                downloaded = 0
                chunk_size = max(1024, self._download_chunk_size)
                max_bytes = max(1024 * 1024, self._download_max_bytes)
                ...
                try:
                    async for chunk in resp.content.iter_chunked(chunk_size):
                        ...
                        if downloaded > max_bytes:
                            logger.warning(
                                "QQ download exceeded max_bytes={} url={} -> abort", max_bytes, url
                            )
                            return None
            ...
            logger.error("QQ download error: {}", exc)
            return None
        finally:
    ...
    async def send(self, msg: "OutboundMessage") -> None:
        ...
        if not self._client:
            logger.warning("QQ client not initialised")
            return
    ...
    async def _send_media(
        ...
        try:
            ...
            logger.error("QQ send media failed filename={} err={}", filename, exc)
            return False

    async def _read_media_bytes(self, media_ref: str) -> tuple[bytes | None, str | None]:
        """Read bytes from http(s) or local file path."""
        import aiohttp
        ...
        if not media_ref.startswith("http://") and not media_ref.startswith("https://"):
            try:
                ...
                logger.warning("QQ outbound media read error ref={} err={}", media_ref, exc)
                return None, None
        ...
        try:
            async with self._http.get(media_ref, allow_redirects=True) as resp:
                if resp.status >= 400:
                    logger.warning(
                        "QQ outbound media download failed status={} url={}", resp.status, media_ref
                    )
                    return None, None
            ...
            logger.warning("QQ outbound media download error url={} err={}", media_ref, exc)
            return None, None
    ...
    async def _post_base64file(
        ...
        if not self._client:
            raise RuntimeError("QQ client not initialised")
